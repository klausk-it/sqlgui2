...
def _workflow_ketten_fenster_oeffnen(rel_id_str, projektname):
    ...
    # ── Schrittweise-Ausführen-Fenster ─────────────────────────────────────────
    _schr_win = [None]   # Toplevel-Referenz
    _schr_idx = [0]      # aktuell angezeigter Schritt (0-basiert)
    _schr_w   = {}       # Widget-Referenzen

    def _aktuellen_quellwert_holen():
        """Gibt den QF-Wert der gerade gewählten Quellzeile zurück."""
        sel = quell_tv.selection()
        if not sel:
            return None
        werte = quell_tv.item(sel[0], "values")
        if qf in quell_spalten:
            _qi = quell_spalten.index(qf)
            return werte[_qi] if _qi < len(werte) else None
        return werte[0] if werte else None

    def _schr_laden(quell_wert, idx=None):
        """Lädt den Ergebnissatz für einen einzelnen Kettenschritt."""
        if not (_schr_win[0] is not None and _schr_win[0].winfo_exists()):
            return
        if not kette_liste:
            return
        if idx is None:
            idx = _schr_idx[0]
        idx = max(0, min(idx, len(kette_liste) - 1))
        _schr_idx[0] = idx
        schritt = kette_liste[idx]
        # Navigationsleiste aktualisieren
        if "btn_zurück" in _schr_w:
            _schr_w["btn_zurück"].config(state="normal" if idx > 0 else "disabled")
            _schr_w["lbl"].config(
                text=f"Schritt {idx+1}/{len(kette_liste)}: {schritt.get('zu_tab','?')}")
            _schr_w["btn_weiter"].config(
                state="normal" if idx < len(kette_liste)-1 else "disabled")
        if not quell_wert:
            if "tv" in _schr_w:
                _schr_w["tv"].delete(*_schr_w["tv"].get_children())
            if "titel_lbl" in _schr_w:
                _schr_w["titel_lbl"].config(
                    text="Bitte im Kettenfenster eine Zeile auswählen")
            return
        try:
            _vb_s = sqlite_verbindung_oeffnen()
            _als  = [f"_ks{i}" for i in range(len(kette_liste) + 1)]
            _joins_s = " ".join(
                f"LEFT JOIN {sql_identifier(s['zu_tab'])} {_als[i+1]} "
                f"ON {_als[i+1]}.{sql_identifier(s['zu_feld'])} "
                f"= {_als[i]}.{sql_identifier(s['von_feld'])}"
                for i, s in enumerate(kette_liste[:idx+1])
            )
            _felder_s = list(schritt.get("felder", [])) or _get_felder_kf(
                schritt.get("zu_tab", ""))
            if _felder_s:
                _sel_s = ", ".join(
                    f"{_als[idx+1]}.{sql_identifier(f)}" for f in _felder_s)
            else:
                _sel_s   = f"{_als[idx+1]}.*"
                _felder_s = [r[1] for r in _vb_s.execute(
                    f"PRAGMA table_info({sql_identifier(schritt.get('zu_tab',''))})"
                ).fetchall()]
            _sql_s = (
                f"SELECT {_sel_s} FROM {sql_identifier(qt)} {_als[0]} "
                f"{_joins_s} WHERE {_als[0]}.{sql_identifier(qf)}=?")
            _cur_s = _vb_s.cursor()
            _cur_s.execute(_sql_s, (quell_wert,))
            _zeilen_s = _cur_s.fetchall()
            _vb_s.close()
        except Exception as _e_s:
            messagebox.showwarning("Schrittweise Ausführen",
                                   f"Fehler beim Laden:\n{_e_s}",
                                   parent=_schr_win[0])
            return
        if "tv" not in _schr_w:
            return
        _schr_tv = _schr_w["tv"]
        if list(_schr_tv["columns"]) != _felder_s:
            _schr_tv.configure(columns=_felder_s)
            for _sp_s in _felder_s:
                _schr_tv.heading(_sp_s, text=_sp_s, anchor="w")
                _schr_tv.column(_sp_s, width=80, anchor="w", minwidth=40, stretch=False)
        _schr_tv.delete(*_schr_tv.get_children())
        for _z_s in _zeilen_s:
            _schr_tv.insert("", "end",
                            values=[str(v) if v is not None else "" for v in _z_s])
        _tv_spalten_auto_breite(_schr_tv, _felder_s, _zeilen_s)
        if "titel_lbl" in _schr_w:
            _schr_w["titel_lbl"].config(
                text=f"Tabelle: {schritt.get('zu_tab','?')}  —  {len(_zeilen_s)} Einträge")

    def _schr_fenster_oeffnen():
        """Öffnet das Schrittweise-Ausführen-Fenster rechts neben dem Kettenfenster."""
        if _schr_win[0] is not None and _schr_win[0].winfo_exists():
            _schr_win[0].lift()
            _schr_win[0].focus_force()
            return
        sw = tk.Toplevel(root)
        _schr_win[0] = sw

        def _schr_schliessen():
            _schr_win[0] = None
            _schr_w.clear()
            try:
                sw.destroy()
        ...
        sw.title(f"⛓ Schrittweise: {bez}  —  {projektname}")
        sw.minsize(320, 240)
        sw.geometry("540x660")
        fenster_registrieren(sw, "Schrittweise", bez)
        fenster_standard_menue_anbringen(sw, "540x660", f"Schrittweise: {bez}")
        sw.title(f"⛓ Schrittweise: {bez}  —  {projektname}")
        sw.protocol("WM_DELETE_WINDOW", _schr_schliessen)

        def _positionieren():
            try:
                _x = win.winfo_x() + win.winfo_width() + 8
                _y = win.winfo_y()
                sw.geometry(f"540x660+{_x}+{_y}")
            except Exception:
                pass
        sw.after(80, _positionieren)

        # ── Navigations-Buttons ──────────────────────────────────────────────
        _btn_frame = tk.Frame(sw)
        _btn_frame.pack(fill="x", padx=6, pady=(6, 2))
        _btn_z = tk.Button(_btn_frame, text="◀ Zurück",
            command=lambda: _schr_laden(
                _aktuellen_quellwert_holen(), _schr_idx[0] - 1))
        _btn_z.pack(side="left", padx=(0, 4))
        _lbl = tk.Label(_btn_frame, text="Kein Schritt", width=30, anchor="center",
                        font=("Segoe UI", 9, "bold"))
        _lbl.pack(side="left", expand=True, fill="x", padx=4)
        _btn_w = tk.Button(_btn_frame, text="Weiter ▶",
            command=lambda: _schr_laden(
                _aktuellen_quellwert_holen(), _schr_idx[0] + 1))
        _btn_w.pack(side="right", padx=(4, 0))
        _schr_w["btn_zurück"] = _btn_z
        _schr_w["lbl"]        = _lbl
        _schr_w["btn_weiter"] = _btn_w

        # ── Titel-Label ──────────────────────────────────────────────────────
        _titel_lbl = tk.Label(sw,
            text="Bitte im Kettenfenster eine Zeile auswählen",
            anchor="w", font=("Segoe UI", 9))
        _titel_lbl.pack(fill="x", padx=6, pady=(0, 2))
        _schr_w["titel_lbl"] = _titel_lbl

        # ── Treeview ─────────────────────────────────────────────────────────
        _tv_frame_s = tk.Frame(sw)
        _tv_frame_s.pack(fill="both", expand=True, padx=6, pady=(0, 6))
        _tv_frame_s.columnconfigure(0, weight=1)
        _tv_frame_s.rowconfigure(0, weight=1)
        _schr_tv_w = ttk.Treeview(_tv_frame_s, columns=(), show="headings",
                                   selectmode="browse")
        _s_sy = ttk.Scrollbar(_tv_frame_s, orient="vertical",   command=_schr_tv_w.yview)
        _s_sx = ttk.Scrollbar(_tv_frame_s, orient="horizontal", command=_schr_tv_w.xview)
        _schr_tv_w.configure(yscrollcommand=_s_sy.set, xscrollcommand=_s_sx.set)
        _schr_tv_w.grid(row=0, column=0, sticky="nsew")
        _s_sy.grid(row=0, column=1, sticky="ns")
        _s_sx.grid(row=1, column=0, sticky="ew")
        _schr_w["tv"] = _schr_tv_w

        # Initialen Stand laden
        _schr_laden(_aktuellen_quellwert_holen(), 0)
        sw.focus_set()
    ...
    quell_tv.bind("<<TreeviewSelect>>", _on_quell_select_erw)
